...
export async function POST(request: NextRequest) {
  try {
    ...
    const messages = [
      { role: 'user' as const, parts: [{ text: promptContent }] }
    ];
    console.log('Bible Explainer - Messages sent to Gemini:', JSON.stringify(messages, null, 2));

    const response = await callGeminiWithFallback(messages);
    console.log('Bible Explainer - Raw Gemini Response:', response);
    ...
    if (!sectionedResponse || sectionedResponse.length === 0) {
      console.error('Bible Explainer - No sections parsed from Gemini response.');
